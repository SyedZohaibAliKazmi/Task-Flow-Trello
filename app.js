...
const myCreateCard = (cardTitle) => {
  ...
  myDiv.addEventListener("drop", (event) => {
    ...
    if (DropElement.className.includes("column")) {
      console.log("2");
      DropElement.appendChild(PickedElement);
    }

    if (DropElement.className.includes("ticket")) {
      DropElement.parentElement.appendChild(PickedElement);
    }
  });
...
const createTicket = (value) => {
  ...
  // if(value==="") return null; // input agar empty ho

  ticket.setAttribute("draggable", "true");
  ticket.setAttribute("class", "ticket"); // drag and drop use
  ...
  ticket.addEventListener("mousedown", (event) => {
    ...
    console.log("1");
    // PickedElement.insertBefore(ticket,myForm);
  });
